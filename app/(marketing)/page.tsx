...
import {
  Globe, Monitor, Download, CheckCircle2, ArrowLeft, RotateCw,
  DollarSign, BarChart3, Percent, ShieldCheck, Zap, LayoutList,
...
export const metadata: Metadata = {
  title: "RealVerdict — Rental property analysis that works",
  description:
    "Analyze any rental property in seconds. Browse Zillow or Redfin inside the app — cap rate, DSCR, cash flow, and your maximum offer price appear automatically.",
}
...
function AppMockup() {
  return (
    <div
      ...
      style={{
        boxShadow:
          "0 32px 96px oklch(0 0 0 / 10%), 0 8px 32px oklch(0 0 0 / 6%), 0 0 0 1px oklch(0 0 0 / 7%)",
        background: "var(--rv-toolbar)",
      }}
    >
      {/* Window title bar */}
      <div
        className="flex items-center h-10 px-4 gap-3 border-b"
      ...
      >
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          <div className="h-5 w-5 rounded flex items-center justify-center opacity-25">
            <ArrowLeft className="h-3 w-3" />
          </div>
          <div className="h-5 w-5 rounded flex items-center justify-center opacity-25">
            <ArrowLeft className="h-3 w-3 scale-x-[-1]" />
          </div>
          <div className="h-5 w-5 rounded flex items-center justify-center opacity-40">
            <RotateCw className="h-3 w-3" />
          </div>
        </div>
        <div
          className="flex-1 h-6 rounded-lg flex items-center gap-2 px-3 max-w-[480px] mx-auto"
          style={{ background: "oklch(0 0 0 / 5%)" }}
        >
          <div className="h-1.5 w-1.5 rounded-full shrink-0" style={{ background: "var(--rv-accent)" }} />
          <span className="text-[11px] font-mono truncate" style={{ color: "var(--rv-t3)" }}>
            zillow.com/homedetails/4821-barton-creek-austin-tx-78735
          </span>
        </div>
      ...
      <div className="flex" style={{ height: "420px" }}>
        ...
        >
          ...
          <div
            ...
            style={{ background: "var(--rv-fill-3)", borderLeft: "2px solid var(--rv-accent)" }}
          >
        ...
        <div className="flex-1 relative overflow-hidden" style={{ background: "#fff" }}>
          <div className="p-5 space-y-3 pointer-events-none" style={{ filter: "blur(1.5px)", opacity: 0.55 }}>
            ...
            <div className="flex gap-3 mt-1">
              {[20, 20, 24].map((w, i) => (
                <div key={i} className={`h-4 w-${w} rounded`} style={{ background: "oklch(0 0 0 / 5%)" }} />
              ))}
            </div>
            <div className="h-40 rounded-lg mt-2" style={{ background: "oklch(0 0 0 / 6%)" }} />
            <div className="grid grid-cols-3 gap-2">
              {[0, 1, 2].map((i) => (
                <div key={i} className="h-24 rounded-md" style={{ background: "oklch(0 0 0 / 4%)" }} />
              ))}
            </div>
            <div className="space-y-2">
              {[1, "5/6", "3/4"].map((w, i) => (
                <div key={i} className={`h-3 w-${w === 1 ? "full" : w} rounded`} style={{ background: "oklch(0 0 0 / 4%)" }} />
              ))}
            </div>
          </div>
          <div
        ...
        {/* DossierPanel */}
        <div
          className="w-60 shrink-0 flex flex-col border-l overflow-hidden"
          style={{ background: "var(--rv-surface-bg)", borderColor: "var(--rv-fill-border)" }}
        >
          <div
            className="h-9 shrink-0 flex items-center px-3 border-b gap-2"
            style={{ borderColor: "var(--rv-fill-border)", background: "var(--rv-toolbar)" }}
          >
          ...
          <div className="px-4 pt-3.5 pb-3 border-b" style={{ borderColor: "var(--rv-fill-border)" }}>
            ...
            <p className="text-[10px] mt-2 leading-snug" style={{ color: "var(--rv-t2)" }}>
              Solid returns at the asking price.
            </p>
          </div>
          ...
          <div className="px-4 py-3 border-b" style={{ borderColor: "var(--rv-fill-border)" }}>
            <div className="grid grid-cols-3 gap-2">
              {[
                { label: "DSCR", value: "1.24", sub: "comfortable", color: "var(--rv-good)" },
                { label: "CAP", value: "6.2%", sub: "above floor", color: "var(--rv-t1)" },
                { label: "CASH", value: "+$340", sub: "/ month", color: "var(--rv-good)" },
              ].map(({ label, value, sub, color }) => (
                <div key={label} className="space-y-1">
                  <p className="text-[8px] font-medium uppercase tracking-widest" style={{ color: "var(--rv-t3)" }}>{label}</p>
                  <p className="text-[17px] font-mono font-medium leading-none" style={{ color, letterSpacing: "-0.01em" }}>{value}</p>
                  <p className="text-[8px]" style={{ color: "var(--rv-t3)" }}>{sub}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="px-4 py-3 border-b" style={{ borderColor: "var(--rv-fill-border)" }}>
            <p className="text-[8px] font-medium uppercase tracking-widest mb-1.5" style={{ color: "var(--rv-t3)" }}>Max offer</p>
            <p className="text-[22px] font-mono font-semibold leading-none" style={{ color: "var(--rv-t1)", letterSpacing: "-0.025em" }}>$412,000</p>
            <p className="text-[8px] mt-1" style={{ color: "var(--rv-t3)" }}>highest price that clears all thresholds</p>
          </div>
...
const steps = [
  {
    ...
    body: "Browse Zillow, Redfin, Realtor.com, or any site in the built-in browser. On web, paste a URL. The moment you land on a property page, the panel wakes up.",
  },
  {
    n: "02",
    title: "Numbers appear",
    body: "The engine reads price, rent estimate, taxes, HOA, and insurance directly from the page — then runs cap rate, DSCR, cash-on-cash, and max offer price automatically.",
  },
  {
    n: "03",
    title: "Make a better offer",
    body: "See exactly what the deal can carry, a clear pass/warn/fail across five criteria, and every assumption you can tweak. No spreadsheet. No guessing.",
  },
]

const webFeatures = [
  "Cap rate, cash-on-cash, DSCR, GRM",
  "Max offer price (highest price that still clears)",
  ...
  return (
    <div className="min-h-screen flex flex-col" style={{ background: "var(--rv-surface-bg)" }}>
      ...
      <main className="flex-1">
        ...
        {/* ── Hero ──────────────────────────────────────────────── */}
        <section className="mx-auto w-full max-w-4xl px-6 pt-20 pb-10 text-center">
          <div
          ...
          <h1
            className="text-[44px] sm:text-[64px] lg:text-[72px] font-bold leading-[1.05] text-balance mx-auto"
          ...
          <p
            className="mt-6 text-[17px] sm:text-[19px] leading-relaxed mx-auto text-balance"
            style={{ color: "var(--rv-t2)", maxWidth: "50ch" }}
          >
            Browse any listing in the built-in browser. Cap rate, DSCR, cash flow,
            and your exact maximum offer price appear in the panel beside the page —
            automatically.
          </p>
          ...
          <div className="mt-9 flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              href="/download"
              className="inline-flex items-center justify-center gap-2 h-12 px-7 rounded-full text-[14px] font-semibold text-white transition-opacity hover:opacity-90 w-full sm:w-auto"
            ...
            <Link
              href="/research"
              className="inline-flex items-center justify-center gap-2 h-12 px-7 rounded-full text-[14px] font-semibold transition-colors hover:bg-[var(--rv-fill-1)] w-full sm:w-auto"
              style={{
                color: "var(--rv-t1)",
          ...
          <p className="mt-4 text-[12px]" style={{ color: "var(--rv-t4)" }}>
            macOS 12+ · Apple Silicon &amp; Intel · No account required to start
          </p>
        </section>
        ...
        <section className="mx-auto w-full max-w-5xl px-6 py-24 sm:py-32">
          <div className="text-center mb-16">
            ...
            <h2
              className="text-[32px] sm:text-[44px] font-bold leading-[1.1]"
              style={{ color: "var(--rv-t1)", letterSpacing: "-0.028em" }}
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-8 sm:gap-10">
            {steps.map((s) => (
              <div key={s.n} className="space-y-4">
                <p
                  className="text-[44px] font-bold font-mono leading-none"
                  style={{ color: "var(--rv-fill-border-strong)", letterSpacing: "-0.04em" }}
                >
                  {s.n}
                </p>
                <div className="w-8 h-px" style={{ background: "var(--rv-accent)" }} />
                <p
        ...
        {/* ── Web vs Desktop ─────────────────────────────────────── */}
        <section className="py-24 sm:py-32" style={{ background: "var(--rv-surface-2)" }}>
          <div className="mx-auto w-full max-w-5xl px-6">
            <div className="text-center mb-14">
              ...
              <p className="mt-4 text-[15px] leading-relaxed mx-auto" style={{ color: "var(--rv-t2)", maxWidth: "46ch" }}>
                Both analyze deals fully. The desktop app adds a native browser so you
                never leave the app — just browse and the numbers appear.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 max-w-3xl mx-auto">
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: "0 1px 4px oklch(0 0 0 / 3%)",
                }}
              >
                ...
                >
                  Open web app
                  <ArrowLeft className="h-3.5 w-3.5 scale-x-[-1]" />
                </Link>
              </div>
              ...
              <div
                ...
                style={{
                  ...
                  boxShadow: "0 4px 20px var(--rv-accent-subtle)",
                }}
              >
                <div
                ...
                <ul className="space-y-2.5">
                  {[...webFeatures, ...desktopExtras].map((f, i) => (
                    <li
                      ...
                      style={{
                        color: i >= webFeatures.length ? "var(--rv-t1)" : "var(--rv-t2)",
                        fontWeight: i >= webFeatures.length ? 500 : 400,
                      }}
                    >
                      <CheckCircle2 className="h-4 w-4 shrink-0 mt-0.5" style={{ color: "var(--rv-accent)" }} />
                      {f}
        ...
        {/* ── Final CTA ──────────────────────────────────────────── */}
        <section className="mx-auto w-full max-w-3xl px-6 py-28 sm:py-36 text-center space-y-8">
          <h2
          ...
          <p className="text-[16px] leading-relaxed mx-auto" style={{ color: "var(--rv-t2)", maxWidth: "38ch" }}>
            Every offer without a clear analysis is a guess. RealVerdict turns any listing
            into a precise, data-backed verdict — in seconds.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 pt-2">
            <Link
              ...
              style={{ background: "var(--rv-accent)", boxShadow: "0 6px 24px var(--rv-accent-border)" }}
            >
              <Download className="h-4 w-4" />
              Download for Mac
            </Link>
